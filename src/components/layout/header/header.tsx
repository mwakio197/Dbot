    ...
    return (
        ...
        >
            <Wrapper variant='left'>
                ...
                <MobileMenu />
                <InfoIcon />
                <buttonlassName="auth-modal-overlay">
                    className="app-header__toggle"modal">
                    onClick={handleToggle}
                    aria-pressed={isToggled}ails</h3>
                >           <button 
                    <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">       className="auth-modal__close-btn" 
                ...
                </button>  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            </Wrapper>41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z" fill="currentColor"/>
                        ...
                        {activeLoginid ? (
                            <>
                                ...
                                <div className="copy-trading-controls">      onClick={() => {
                                    ...
                                    {/* Only show tokens section if copy trading is enabled */}  type="text" 
                                    {copyTradeEnabled && (      placeholder="Enter token..."
                                        <>value={tokenInput}
                                            <div className="input-section">kenInput(e.target.value)}
                                                <input => {
                                                    type="text"  'Enter') {
                                                    placeholder="Enter token..."               submitToken(e);
                                                    value={tokenInput}
                                                    onChange={(e) => setTokenInput(e.target.value)}
                                                    onKeyPress={(e) => {assName="auth-modal__input"
                                                        if (e.key === 'Enter') {
                                                            submitToken(e);  <button 
                                                        }on" 
                                                    }}
                                                />
                                                <button 
                                                    className="submit-button" 00/svg">
                                                    title="Submit token"4L2.01 21Z"/>
                                                    onClick={submitToken}
                                                >
                                                    <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M2.01 21L23 12L2.01 3L2 10L17 12L2 14L2.01 21Z"/>nnected-tokens">
                                                    </svg>onnected-tokens__title">Connected Tokens</h4>
                                                </button>
                                            </div>
                                            <div className="connected-tokens">div key={index} className="connected-tokens__item">
                                                <h4 className="connected-tokens__title">Connected Tokens</h4>
                                                <div className="connected-tokens__list">
                                                    {tokens.map((token, index) => (  {token}
                                                        <div key={index} className="connected-tokens__item">/span>
                                                            <div className="connected-tokens__item-info">  </div>
                                                                <span className="connected-tokens__item-token">         <button 
                                                                    {token}              className="connected-tokens__item-remove"
                                                                </span>                  onClick={() => removeToken(token)}
                                                            </div>                     >
                                                            <button                               <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                                className="connected-tokens__item-remove"                                  <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/>
                                                                onClick={() => removeToken(token)}svg>
                                                            >button>
                                                                <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    )}assName="copy-to-real__title">Demo Settings</h4>
                                </div>div className="copy-to-real__toggle">
                                {client.loginid?.startsWith('VR') && (
                                    <div className="copy-to-real">on 
                                        <h4 className="copy-to-real__title">Demo Settings</h4>      className={`toggle-button ${copyToReal ? 'active' : ''}`}
                                        <div className="copy-to-real__toggle">          onClick={() => {
                                            <span>Copy trades to real account</span>                      const newValue = !copyToReal;
                                            <button                          setCopyToReal(newValue);
                                                className={`toggle-button ${copyToReal ? 'active' : ''}`}                           localStorage.setItem(`copytoreal_${client.loginid}`, newValue.toString());
                                                onClick={() => {
                                                    const newValue = !copyToReal;
                                                    setCopyToReal(newValue);an>
                                ...
                                )} className="no-account-state__icon">
                            </>http://www.w3.org/2000/svg">
                        ) : ("M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-4h2v2h-2zm0-2h2V7h-2z"/>
                            <div className="no-account-state">
                                <div className="no-account-state__icon">
                                    ...
                                    </svg>account to start copy trading
                                </div>p>
                                <h3 className="no-account-state__title">No Active Account</h3>   <button 
                                <p className="no-account-state__description">me="no-account-state__button auth-modal__button auth-modal__button--primary"
                                    Please log in or create an account to start copy tradingnClick={() => {
                                </p>          const app_id = getAppId();
                                <button                   const oauth_url = 'https://oauth.deriv.com/oauth2/authorize';
                                    className="no-account-state__button"                  const redirect_uri = encodeURIComponent(`${window.location.origin}/callback`);
                                    onClick={() => {                    const url = `${oauth_url}?app_id=${app_id}&l=EN&brand=deriv&redirect_uri=${redirect_uri}`;
                                        const app_id = getAppId();                              window.location.href = url;
                                        const oauth_url = 'https://oauth.deriv.com/oauth2/authorize';                       }}
                                        const redirect_uri = encodeURIComponent(`${window.location.origin}/callback`);                              >
                                        const url = `${oauth_url}?app_id=${app_id}&l=EN&brand=deriv&redirect_uri=${redirect_uri}`;                                     Get Started
                                        window.location.href = url;                                    </button>
                                    }}       </div>
...
export default AppHeader;});    );        </Header>            )}                </Modal>                    </div>                        )}                            </div>                                </button>                                    Get Started                                >                            )}
                        </div>
